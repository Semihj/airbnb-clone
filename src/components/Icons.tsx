...
export default function Icons() {
  ...
  return (
    <div className=" w-full   flex fixed h-[100px]  bg-white z-[100] left-0 border-b-2 mt-[75px] md:mt-[100px] ">
      ...
      >
        <button
          className={`sticky left-0  hidden md:inline-block cursor-pointer text-[24px] p-2 rounded-full right-arrow bg-white  border-2 ${
            rowRef?.current?.scrollLeft === 0 && "hidden"
          }  z-10`}
          onClick={scrollLeft}
        >
          <FaAngleLeft />
        ...
        <button
          className="sticky  right-0 hidden md:inline-block cursor-pointer text-[24px] p-2 rounded-full right-arrow bg-white border-2  z-10  "
          onClick={scrollRight}
        >
          <FaAngleRight />
        </button>
